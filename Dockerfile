...
RUN --mount=type=cache,id=nuget,target=/root/.nuget/packages \
    dotnet publish TaskFlow.Api.csproj -a ${TARGETARCH/amd64/x64} --use-current-runtime --self-contained false -o /app
...
# Store the DB in a dedicated directory so it can be volume-mounted
RUN mkdir -p /app/data && chown $APP_UID /app/data

ENV ConnectionStrings__DefaultConnection="Data Source=/app/data/taskflow.db"

# Switch to a non-privileged user AFTER setup is done
USER $APP_UID
